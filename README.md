...
For a production-grade example of using mcp-chaos-monkey (TypeScript), see [reliable-mcp](https://github.com/alexey-tyurin/reliable-mcp) — an MCP Reliability Playbook that uses this framework for chaos testing with 21 automated fault injection scenarios.

## Features

- **8 fault types:** latency, error, timeout, malformed, connection-refused, connection-drop, rate-limit, schema-mismatch
- **Transport-level interception:** HTTP (fetch/httpx), Redis, auth middleware (Express/ASGI)
- **Production safety:** hard guards prevent running in production
- **Zero runtime dependencies** (interceptors use optional deps)
- **Pluggable logging:** bring your own logger (pino, structlog, stdlib logging)
...
- **TypeScript:** See the [TypeScript README](typescript/README.md) for installation and usage.
- **Python:** See the [Python README](python/README.md) for installation and usage.
